...
impl fmt::Display for ParseHexError {
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ParseHexError::Range { ref min, ref max , ref got} => {
                write!(f,"expected buff size in `{}...{}`, got `{}`",min,max,got)
            },
            ParseHexError::Size { ref expect, ref actual } => {
                write!(f,"expected buff size `{}` got `{}`",expect,actual)
            },
            ParseHexError::Char { ref val } => {
                write!(f,"non-hex character `{}`",val)
            }
        }
    }
}
