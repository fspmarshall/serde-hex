...
use serde_hex::{SerHexSeq,SerHexOpt,StrictPfx};
...
#[test]
fn serialize() {
    let ext = Ext { seq: vec![0xde,0xad,0xbe,0xef], opt: Some(0xff) };
...
fn deserialize() {
    ...
    let exp = Ext { seq: vec![0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef], opt: Some(0xaa) };
    assert_eq!(ext,exp);
}
